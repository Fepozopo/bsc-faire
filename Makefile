...
# Build for x86_64 CLI (binary works on all platforms due to CGO_ENABLED=0) 
x86_64-cli:
...
# Build for ARM CLI (binary works on all platforms due to CGO_ENABLED=0)
arm-cli:
	@echo "Building CLI for ARM with GOOS=linux, GOARCH=arm64..."
	$(call MAKE_BIN_DIR)
...
all: cli gui x86_64-cli arm-cli windows-x86_64-gui windows-arm-gui linux-x86_64-gui linux-arm-gui macos-arm-gui
...
.PHONY: cli gui x86_64-cli arm-cli windows-x86_64-gui windows-arm-gui linux-x86_64-gui linux-arm-gui macos-arm-gui all clean